...
export default function UnifiedReport() {
  ...
  const taskTypeData = useMemo(() => [
    { name: "FMS Tasks", value: summary.actualToPlannedCount + summary.plannedToPlannedCount, color: CHART_COLORS.fms },
    { name: "Regular Tasks", value: Math.max(0, summary.totalTasks - (summary.actualToPlannedCount + summary.plannedToPlannedCount)), color: CHART_COLORS.regular },
  ], [summary]);
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-medium">
                  Executive User Overview & Visual Metrics Across Workflows & Delegated Tasks
                </p>
              </div>
            </div>
        ...
        <CardContent className="pt-4 space-y-5">
              ...
              <div className="w-48">
                ...
                <AntdSelect
                  ...
                  options={[
                    ...
                    ...userOptions.map((u) => ({ value: u._id, label: u.name })),
                  ]}
                />
              </div>
              ...
              <div className="w-44">
                ...
                <AntdSelect
                  ...
                  options={[
                    ...
                    ...templates.map((t) => ({ value: t._id, label: t.templateName || t.fmsId })),
                  ]}
                />
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
            <div className="p-3.5 rounded-xl bg-blue-50/60 border border-blue-100">
              <span className="text-[10px] font-bold text-blue-600 uppercase block">Total Tasks</span>
              <h3 className="text-2xl font-black text-gray-900 mt-1">{summary.totalTasks}</h3>
            </div>

            <div className="p-3.5 rounded-xl bg-emerald-50/60 border border-emerald-100">
              <span className="text-[10px] font-bold text-emerald-600 uppercase block">On-Time Done</span>
              <h3 className="text-2xl font-black text-emerald-800 mt-1">{summary.onTime}</h3>
            </div>

            <div className="p-3.5 rounded-xl bg-amber-50/60 border border-amber-100">
              <span className="text-[10px] font-bold text-amber-600 uppercase block">Late Done</span>
              <h3 className="text-2xl font-black text-amber-800 mt-1">{summary.late}</h3>
            </div>

            <div className="p-3.5 rounded-xl bg-red-50/60 border border-red-100">
              <span className="text-[10px] font-bold text-red-600 uppercase block">Overdue</span>
              <h3 className="text-2xl font-black text-red-800 mt-1">{summary.overdue}</h3>
            </div>

            <div className="p-3.5 rounded-xl bg-gray-100/80 border border-gray-200">
              <span className="text-[10px] font-bold text-gray-600 uppercase block">Not Done</span>
              <h3 className="text-2xl font-black text-gray-800 mt-1">{summary.notDone}</h3>
            </div>

            <div className="p-3.5 rounded-xl bg-purple-50/60 border border-purple-100">
              <span className="text-[10px] font-bold text-purple-600 uppercase block">Actual-To-Planned</span>
              <h3 className="text-2xl font-black text-purple-900 mt-1">{summary.actualToPlannedCount}</h3>
            </div>

            <div className="p-3.5 rounded-xl bg-indigo-50/60 border border-indigo-100">
              <span className="text-[10px] font-bold text-indigo-600 uppercase block">Planned-To-Planned</span>
              <h3 className="text-2xl font-black text-indigo-900 mt-1">{summary.plannedToPlannedCount}</h3>
            </div>
          </div>
            ...
            <Card className="border border-gray-200/80 shadow-2xs bg-white">
              ...
              <CardContent className="p-3 h-52 flex items-center justify-center">
                {summary.totalTasks > 0 ? (
                  ...
                  <div className="text-center text-xs text-gray-400">No status data</div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border border-gray-200/80 shadow-2xs bg-white">
              ...
              <CardContent className="p-3 h-52 flex items-center justify-center">
                {summary.totalTasks > 0 ? (
                  ...
                  <div className="text-center text-xs text-gray-400">No volume data</div>
                )}
              </CardContent>
            </Card>
                ...
                {topUserLeaderboard.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <YAxis dataKey="userName" type="category" tick={{ fontSize: 9 }} width={65} />
                      <RechartsTooltip formatter={(value) => [`${value}%`, "Completion Rate"]} />
                      <Bar dataKey="rate" fill="#10B981" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="text-center text-xs text-gray-400">No leaderboard stats</div>
                )}
              </CardContent>
            </Card>
          </div>
              ...
              <Table>
                <TableHeader className="bg-gray-50">
                  <TableRow>
                    ...
                    <TableHead className="text-xs font-bold text-center">ON TIME</TableHead>
                    <TableHead className="text-xs font-bold text-center">LATE</TableHead>
                    <TableHead className="text-xs font-bold text-center">OVERDUE</TableHead>
                    <TableHead className="text-xs font-bold text-center">NOT DONE</TableHead>
                ...
                <TableBody>
                  {loading ? (
                    <TableRow>
                      <TableCell colSpan={8} className="p-8 text-center"><Spin /></TableCell>
                    </TableRow>
                  ) : filteredUserSummary.length > 0 ? (
                    filteredUserSummary.map((u) => (
                      <TableRow key={u.userId} className="hover:bg-gray-50 text-xs">
                        <TableCell className="p-3 font-semibold text-gray-900">
                          {u.userName} <span className="text-[10px] text-gray-400 font-mono">({u.employeeCode})</span>
                        </TableCell>
                        <TableCell className="text-center font-bold text-gray-900">{u.total}</TableCell>
                        <TableCell className="text-center font-bold text-emerald-600">{u.completed}</TableCell>
                        <TableCell className="text-center font-bold text-emerald-700">{u.onTime}</TableCell>
                        <TableCell className="text-center font-bold text-amber-600">{u.late}</TableCell>
                        <TableCell className="text-center font-bold text-red-600">{u.overdue}</TableCell>
                        <TableCell className="text-center font-bold text-gray-600">{u.notDone}</TableCell>
